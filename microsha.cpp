...
template <typename T>
vector<T> summ_massive(const vector<T> &str1 ,const vector<T> &str2) {
  vector<T> res ;
...
vector<pair<string, int>> path_to_mass(const string& path ,const string& regex ) {
    //if( path == "") path = "/";
...
vector<pair<string, int>> dir_path(vector<string>& mass) {
	//cout << "mass: " ;
	//cout << mass.size();
	...
	for(int i = 0 ; i < mass.size() ; ++i) {
		...
		if( mass[i] != "/" && mass[i] != ".") {
			for(int j = 0 ; j < mass_path.size() ;++j) {
				...
				temp = path_to_mass(mass_path[j].first , regex);
...
int lsh_ls(char** args) {
	...
	if (argums.size() == 1 || argums.back()[0] == '-') {
		...
		for(auto k : ans ) if (k.second == 1) cout << k.first << endl;
		//cout<<endl;
